...
from amsrag.base import (
    BaseGraphStorage,
    BaseVectorStorage,
    TextChunkSchema,
)
from amsrag.answer_generation.prompts import PROMPTS
from amsrag._utils import logger, compute_mdhash_id
...
def _require_dspy() -> None:
    if dspy is None:
        raise RuntimeError(
            "DSPy is required for entity extraction, but it failed to initialize. "
            "Install/fix DSPy and re-run extraction."
        ) from _DSPY_IMPORT_ERROR
# 这些函数需要重新实现或从其他模块导入
# from amsrag._op import _merge_edges_then_upsert, _merge_nodes_then_upsert
...
async def _merge_nodes_then_upsert(
    ...
    from amsrag._utils import split_string_by_multi_markers
    from amsrag.answer_generation.prompts import GRAPH_FIELD_SEP
...
async def generate_dataset(
    ...
    _require_dspy()
    from amsrag.entity_extraction.module import TypedEntityRelationshipExtractor
...
async def extract_entities_dspy(
    ...
    from amsrag._utils import limit_async_func_call
        ...
        for attempt in range(max_retries + 1):
            try:
                resp = await client.chat.completions.create(
                    model=os.getenv("AMSRAG_ENTITY_EXTRACTION_MODEL", "qwen-flash"), 
                    messages=messages, 
                    temperature=0.1, 
                    max_tokens=8192
                )
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        try:
            ...
            if os.environ.get("DASHSCOPE_API_KEY"):
                ...
                from amsrag._utils import convert_response_to_json
                ...
                from amsrag.entity_extraction.module import Entity as _Entity, Relationship as _Relationship
