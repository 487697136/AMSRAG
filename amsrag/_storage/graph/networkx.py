...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_nodes_batch(self, node_ids: list[str]) -> dict[str, Union[dict, None]]:
        results = await asyncio.gather(*[self.get_node(node_id) for node_id in node_ids])
    ...
    # 添加别名方法以保持接口一致性
    async def add_nodes_batch(self, nodes_data: list[tuple[str, dict[str, str]]]):
        """添加节点批处理 - 别名方法"""
        return await self.upsert_nodes_batch(nodes_data)
    
    async def add_edges_batch(self, edges_data: list[tuple[str, str, dict[str, str]]]):
        """添加边批处理 - 别名方法"""
        return await self.upsert_edges_batch(edges_data)
    ...
    async def _leiden_clustering(self):
        try:
            ...
            use_graspologic = True
        except ImportError:
            logger.warning("graspologic not available, using leidenalg as fallback")
            use_graspologic = False
        ...
        if use_graspologic:
            ...
            ig_graph = ig.Graph(len(node_list), directed=False)
            ig_graph.add_edges([(node_to_idx[u], node_to_idx[v]) for u, v in edges])
            
            # 运行leiden算法
            partition = leidenalg.find_partition(
                ig_graph, 
                leidenalg.RBConfigurationVertexPartition,
                seed=self.global_config["graph_cluster_seed"]
            )
            
            # 转换为node_communities格式（单层）
            node_communities: dict[str, list[dict[str, str]]] = defaultdict(list)
            for node_idx, cluster_id in enumerate(partition.membership):
                node_name = node_list[node_idx]
                node_communities[node_name].append(
                    {"level": 0, "cluster": str(cluster_id)}
                )
            node_communities = dict(node_communities)
            __levels = {0: len(set(partition.membership))}
            logger.info(f"Leidenalg clustering: {__levels[0]} communities")
