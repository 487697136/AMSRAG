...
async def global_query(
    ...
    # from silently failing when community reports were generated with low ratings.
    if not community_datas_filtered and community_datas:
        logger.warning(
            f"All {len(community_datas)} community reports were filtered by min_rating="
            f"{query_param.global_min_community_rating}. Falling back to top-rated available reports."
        )
