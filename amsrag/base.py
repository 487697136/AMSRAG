...
@dataclass
class QueryParam:
    # `auto` is an adaptive routing entrypoint in EnhancedGraphRAG.
    mode: Literal["auto", "local", "global", "global_local", "naive", "bm25", "llm_only"] = "global"
    only_need_context: bool = False
    response_type: str = "多段落回答（中文）"
    level: int = 2
    top_k: int = 20
    # naive search
    naive_max_token_for_text_unit = 12000
    # bm25 search
    bm25_max_token_for_text_unit: int = 12000
    # local search
    ...
    # global search
    # Raise min rating from 0 to filter out empty/meaningless community reports
    global_min_community_rating: float = 1
    ...
    # llm_only search
    llm_only_system_prompt: str = "你是一个专业、准确、公正的问答助手。请基于你的知识回答问题。"
...
class BaseGraphStorage(StorageNameSpace):
    ...
    async def embed_nodes(self, algorithm: str) -> tuple[np.ndarray, list[str]]:
        raise NotImplementedError("Node embedding is not used in amsrag.")
