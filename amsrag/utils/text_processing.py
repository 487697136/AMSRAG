...
class _FallbackTokenizer:
    """
    Minimal tokenizer fallback that avoids any network dependency.

    This is only used when tiktoken model loading fails in restricted environments.
    Token counts are approximate but deterministic.
    """
    ...
    def decode(self, tokens: List[int]) -> str:
        if not tokens:
            return ""
        normalized = [int(t) % 256 for t in tokens]
        return bytes(normalized).decode("utf-8", errors="ignore")

    def encode_batch(self, texts: List[str], num_threads: int = 1) -> List[List[int]]:
        del num_threads
...
def get_tiktoken_encoder(model_name: str = "gpt-4o", fallback_name: str = "cl100k_base"):
    """
    Get a tiktoken encoder with an offline-safe fallback.

    Args:
        model_name: Preferred model name for `encoding_for_model`.
        fallback_name: Fallback encoding name used when model loading fails.
    ...
    try:
        ...
        logger.warning(
            f"Failed to load tokenizer for model '{model_name}': {model_error}. "
        ...
        try:
            ...
            logger.warning(
                f"Failed to load fallback tokenizer '{fallback_name}': {fallback_error}. "
                "Using built-in byte tokenizer fallback."
            )
